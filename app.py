...
class App(ctk.CTk):
    ...
    def tela_login(self):
        ...
        self.usuario_entry = ctk.CTkEntry(frame, placeholder_text="Usuário", width=280)
    ...
    def tela_principal(self):
        ...
        ctk.CTkLabel(
            menu,
            text=f"Olá, {self.usuario_logado['nome']}",
            text_color="white",
            font=("Arial", 12)
        ).pack(pady=(0, 18))
        ...
        ctk.CTkButton(menu, text="Relatórios", command=self.tela_relatorios, **btn_style).pack(pady=8)
    ...
    def tela_produtos(self):
        ...
        self.p_preco = ctk.CTkEntry(form, placeholder_text="Preço", width=120)
        ...
        self.p_cb = ctk.CTkEntry(form, placeholder_text="Código de barras", width=220)
        ...
        self.lista_produtos = ctk.CTkTextbox(self.conteudo, width=960, height=420)
        self.lista_produtos.pack(pady=10, padx=16)
    ...
    def _atualizar_lista_produtos(self, filtros=None):
        ...
        self.lista_produtos.insert("end", "ID | Nome | Categoria | Preço | Quantidade | Mari | Data\n")
        self.lista_produtos.insert("end", "-" * 100 + "\n")
    ...
    def tela_clientes(self):
        ...
        self.flag_sousa = ctk.CTkCheckBox(flags, text="É de Sousa")
        ...
        self.lista_clientes = ctk.CTkTextbox(self.conteudo, width=960, height=250)
        self.lista_clientes.pack(pady=8, padx=16)
        ...
        self.box_pedidos_cliente = ctk.CTkTextbox(self.conteudo, width=960, height=180)
        self.box_pedidos_cliente.pack(pady=8, padx=16)
    ...
    def _atualizar_lista_clientes(self, termo=None):
        ...
        self.lista_clientes.insert("end", "-" * 110 + "\n")
    ...
    def _consultar_cliente_pedidos(self):
        try:
            ...
            self.box_pedidos_cliente.insert("end", "-" * 120 + "\n")
    ...
    def tela_consultas(self):
        ...
        self.f_nome = ctk.CTkEntry(filtros, placeholder_text="Nome contém", width=220)
        ...
        self.f_min = ctk.CTkEntry(filtros, placeholder_text="Preço mínimo", width=120)
        self.f_min.grid(row=0, column=2, padx=8, pady=8)

        self.f_max = ctk.CTkEntry(filtros, placeholder_text="Preço máximo", width=120)
        ...
        self.lista_consultas = ctk.CTkTextbox(self.conteudo, width=960, height=460)
        self.lista_consultas.pack(pady=10, padx=16)
    ...
    def _aplicar_filtros(self):
        try:
            ...
            self.lista_consultas.insert("end", "ID | Nome | Categoria | Preço | Quantidade | Mari\n")
            self.lista_consultas.insert("end", "-" * 90 + "\n")
    ...
    def tela_vendas(self):
        ...
        self.v_cliente_lbl = ctk.CTkLabel(top, text="Cliente: não selecionado", text_color="white")
        ...
        self.v_prod_id = ctk.CTkEntry(mid, placeholder_text="ID do produto", width=140)
        self.v_prod_id.grid(row=0, column=0, padx=8, pady=8)

        self.v_qtd = ctk.CTkEntry(mid, placeholder_text="Quantidade", width=140)
        ...
        self.v_forma = ctk.CTkOptionMenu(mid, values=["cartao", "boleto", "pix", "berries"], width=140)
        self.v_forma.grid(row=0, column=4, padx=8, pady=8)
        self.v_forma.set("pix")

        self.v_status = ctk.CTkOptionMenu(mid, values=["pendente", "confirmado", "cancelado"], width=140)
        self.v_status.grid(row=0, column=5, padx=8, pady=8)
        ...
        self.v_itens = ctk.CTkTextbox(self.conteudo, width=960, height=430)
        self.v_itens.pack(pady=10, padx=16)
    ...
    def _buscar_cliente(self):
        ...
        if not cliente:
            messagebox.showerror("Erro", "Cliente não encontrado. Cadastre o cliente antes da venda.")
            return
    ...
    def _render_carrinho(self):
        ...
        if not self.carrinho:
            ...
            for indice, item in enumerate(self.carrinho, start=1):
                self.v_itens.insert(
                    "end",
                    f"{indice}. Produto ID: {item.produto_id} | Quantidade: {item.quantidade}\n"
                )
    ...
    def tela_relatorios(self):
        ...
        self.r_mes = ctk.CTkEntry(frame, placeholder_text="Mês", width=120)
        self.r_mes.grid(row=0, column=1, padx=8, pady=8)
        self.r_mes.insert(0, str(datetime.now().month))

        ctk.CTkButton(
            frame,
            text="Relatório por vendedor",
            fg_color=COR_DESTAQUE,
            hover_color="#2DA7D7",
            text_color="black",
        ...
        self.r_box = ctk.CTkTextbox(self.conteudo, width=960, height=470)
        self.r_box.pack(pady=10, padx=16)
    ...
    def _relatorio_proc(self):
        try:
            ...
            self.r_box.insert("end", f"Relatório mensal por vendedor — {ano}-{mes:02d}\n")
            self.r_box.insert("end", "-" * 100 + "\n")

            if not dados:
                self.r_box.insert("end", "(sem registros no período)\n")
            else:
                for linha in dados:
                    self.r_box.insert(
                        "end",
    ...
    def _listar_vendas_mes(self):
        try:
            ...
            self.r_box.configure(state="normal")
            self.r_box.delete("1.0", "end")
            self.r_box.insert("end", f"Vendas do mês — {ano}-{mes:02d}\n")
            self.r_box.insert("end", "-" * 100 + "\n")

            if not vendas:
                self.r_box.insert("end", "(sem vendas no período)\n")
            else:
                for venda in vendas:
                    self.r_box.insert(
                        "end",
